...
char **str_tok(char *str, char ch) {
    ...
    char **str_tokd = (char **) malloc(n * sizeof(char *));
    for (int i = 0; i < n; ++i) {
        str_tokd[i] = (char *) malloc(MAX_C);
    }
    int j = 0, k = 0;
    while (*str) {
        if (*str == ch) {
...
char *eval(char *str) {
    ...
    double ans = 0;
    if ((contains_i(str, '(') == 1 && contains_i(str, ')') == 1 && str[0] == '(' && str[str_len(str) - 1] == ')') ||
        (!contains_i(str, '(') && !contains_i(str, ')'))) {
        char *str_ans = (char *) malloc(MAX_C);
        CMPLX_NUM parsedCMPLX = parseComplex(str);
        sprintf(str_ans, "{%lf,%lf}", parsedCMPLX.re, parsedCMPLX.im);
        return str_ans;
    }
    char *str_build = (char *) malloc(MAX_C);
    *str_build = 0;
    while (*str) {
        char ch = *str;
        if (ch != '(' && ch != ')') {
...
CMPLX_NUM parseOneComplexTerm(char *str) {
    ...
    while (*tp) {
        if (*tp == '/') {
            ...
            char *tp2 = tp + 1;
            while (*tp2) {
                if (!isNum(*tp2) && *tp2 != '+' && *tp2 != '-' && *tp2 != 'i' && *tp2 != 'j' && *tp2 != '{' &&
                    *tp2 != ',' && *tp2 != '}' && *tp2 != '.' && *tp2 != '<')
                    break;
                idx++;
                tp2++;
    ...
    for (int i = 0; i < str_len(str); ++i) {
        ...
        if (str[i] == '+' && (isNum(str[i - 1]) || str[i - 1] == 'i' || str[i - 1] == 'j' || str[i - 1] == '}')) {
            ...
            k = 0;
                   ...
                   (isNum(str[i - 1]) || str[i - 1] == 'i' || str[i - 1] == 'j' || str[i - 1] == '}')) {
            str_tokd[j][k] = 0;
            CMPLX_NUM parsed_cmplx = parseOneComplexTerm(str_tokd[j++]);
            ans.re += to_add * parsed_cmplx.re;
            ans.im += to_add * parsed_cmplx.im;
...
CMPLX_NUM det(CMPLX_NUM **mat, int m) {
    ...
    CMPLX_NUM ans = {0, 0};
    for (int k = 0; k < m; ++k) {
        CMPLX_NUM **new_mat = (CMPLX_NUM **) malloc((m - 1) * sizeof(CMPLX_NUM *));
        int r = 0;
        for (int x = 1; x < m; ++x) {
            int c = 0;
            *(new_mat + r) = (CMPLX_NUM *) malloc((m - 1) * sizeof(CMPLX_NUM));
            for (int y = 0; y < m; ++y) {
                if (y == k)
                    continue;
                *(*(new_mat + r) + c++) = *(*(mat + x) + y);
...
int main() {
    ...
    CMPLX_NUM D;
    CMPLX_NUM **mat;
    mat = (CMPLX_NUM **) malloc(m * sizeof(CMPLX_NUM *));
    for (int i = 0; i < m; ++i) {
        *(mat + i) = (CMPLX_NUM *) malloc(m * sizeof(CMPLX_NUM));
        for (int j = 0; j < m; ++j) {
